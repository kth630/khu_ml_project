...
def evaluate_binary_classifier(y_true: pd.Series, pred_proba: np.ndarray) -> dict[str, float]:
    # 0.5 기준으로 예측 class를 만든다.
...
def precision_recall_at_k(y_true: pd.Series, pred_proba: np.ndarray, k: float) -> dict[str, float]:
    # Series/array를 numpy array로 변환한다.
    y_true_arr = np.asarray(y_true).astype(int)
    pred_proba_arr = np.asarray(pred_proba)
    n = len(y_true_arr)

    # 상위 k%에 해당하는 샘플 수를 계산한다.
    top_k_size = max(1, int(np.ceil(n * k)))

    # 예측 확률이 높은 순서대로 정렬한 뒤 상위 k% index를 선택한다.
    order = np.argsort(-pred_proba_arr)[:top_k_size]

    # 전체 positive 수와 상위 k% 안의 positive 수를 계산한다.
    positives = y_true_arr.sum()
    top_positives = y_true_arr[order].sum()

    # base positive rate, Precision@K, Recall@K, Lift@K를 계산한다.
    base_positive_rate = positives / n if n else 0
    precision_at_k = top_positives / top_k_size if top_k_size else 0
    recall_at_k = top_positives / positives if positives else 0
...
def run_model_pipeline(df: pd.DataFrame, target: str, model_name: str) -> dict[str, object]:
    ...
    for split, y_true, pred in [("valid", y_valid, valid_pred), ("test", y_test, test_pred)]:
        ...
        row.update(evaluate_binary_classifier(y_true, pred))
        metric_rows.append(row)

    # validation/test Top-K 성능 지표를 계산한다.
    topk_rows = []
    for split, y_true, pred in [("valid", y_valid, valid_pred), ("test", y_test, test_pred)]:
        for k in [0.1, 0.2, 0.3]:
            row = {"model": model_name, "split": split}
            row.update(precision_recall_at_k(y_true, pred, k))
...
def main() -> None:
    ...
    # split 요약, 기본 성능, Top-K 성능을 하나의 표로 합친다.
